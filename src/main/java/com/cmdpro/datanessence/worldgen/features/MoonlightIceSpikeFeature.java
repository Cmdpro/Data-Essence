...
public class MoonlightIceSpikeFeature extends IceSpikeFeature {
    public MoonlightIceSpikeFeature(Codec<NoneFeatureConfiguration> pCodec) {
        super(pCodec);
    }
                    ...
                    for (int j1 = -l; j1 <= l; j1++) {
                        ...
                        if ((i1 == 0 && j1 == 0 || !(f1 * f1 + f2 * f2 > f * f))
                            ...
                            if (blockstate.isAir() || isDirt(blockstate) || blockstate.is(Blocks.SNOW_BLOCK) || blockstate.is(Blocks.ICE)) {
                                this.setBlock(worldgenlevel, blockpos.offset(i1, k, j1), BlockRegistry.FROZEN_MOONLIGHT.get().defaultBlockState());
                            }
            ...
            for (int l1 = -k1; l1 <= k1; l1++) {
                for (int i2 = -k1; i2 <= k1; i2++) {
                    ...
                    while (blockpos1.getY() > 50) {
                        ...
                        this.setBlock(worldgenlevel, blockpos1, BlockRegistry.FROZEN_MOONLIGHT.get().defaultBlockState());
