...
public class LunarEssenceBatteryRenderer implements BlockEntityRenderer<LunarEssenceBatteryBlockEntity> {
    @Override
    public void render(LunarEssenceBatteryBlockEntity pBlockEntity, float pPartialTick, PoseStack pPoseStack, MultiBufferSource pBuffer, int pPackedLight, int pPackedOverlay) {
        pPoseStack.pushPose();
        ...
        Color color = new Color(1.0f, 1.0f, 1.0f, pBlockEntity.storage.getEssence(EssenceTypeRegistry.LUNAR_ESSENCE.get())/pBlockEntity.storage.getMaxEssence());
    ...
    private static void renderQuad(LunarEssenceBatteryBlockEntity pBlockEntity, Direction pDirection, VertexConsumer builder, PoseStack poseStack, float x0, float y0, float z0, float x1, float y1, float z1, float u0, float v0, float u1, float v1, int packedLight, int color) {
    ...
    public static final ResourceLocation OVERLAY = DataNEssence.locate("block/lunar_essence_battery_overlay");
    public LunarEssenceBatteryRenderer(BlockEntityRendererProvider.Context rendererProvider) {
